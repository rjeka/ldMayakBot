...
def services(message):
    ...
    services_keyboard.add(bulk_button, well_button, electric_button, windows_button)
    ...
    if security.check_user_id(user_info):
        ...
        with con.cursor() as cur:
            ...
            for row in (rows):
                bot_message = ""
                if row[0]:
                    bot_message = bot_message + "Описание:\n" + row[0] + "\n\n"
                if row[1]:
                ...
                time.sleep(1)
        keyboards.main_menu_key(services_keyboard)
        bot.send_message(message.chat.id, "Выполнен запрос по выбранному разделу", reply_markup=services_keyboard)
    else:
